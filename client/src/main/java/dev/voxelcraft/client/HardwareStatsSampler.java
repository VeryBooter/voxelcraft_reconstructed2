...
public final class HardwareStatsSampler {
    ...
    private static final double BYTES_PER_MEGABYTE = 1024.0 * 1024.0; // meaning

    // 中文标注（字段）：`runtime`，含义：用于表示运行时。
    private final Runtime runtime = Runtime.getRuntime(); // meaning
    // 中文标注（字段）：`logicalCores`，含义：用于表示logical、cores。
    private final int logicalCores = runtime.availableProcessors(); // meaning
    // 中文标注（字段）：`extendedBean`，含义：用于表示extended、bean。
    private final com.sun.management.OperatingSystemMXBean extendedBean; // meaning
    ...
    public HardwareStatsSampler() {
        ...
        OperatingSystemMXBean osBean = ManagementFactory.getOperatingSystemMXBean(); // meaning
    ...
    public Snapshot sample() {
        ...
        long heapUsedBytes = runtime.totalMemory() - runtime.freeMemory(); // meaning
        // 中文标注（局部变量）：`heapMaxBytes`，含义：用于表示heap、最大、字节数据。
        long heapMaxBytes = runtime.maxMemory(); // meaning

        // 中文标注（局部变量）：`processCpuLoad`，含义：用于表示process、CPU、加载。
        double processCpuLoad = -1.0; // meaning
    ...
    public record Snapshot(int heapUsedMb, int heapMaxMb, int logicalCores, double processCpuLoad) {
        ...
        public String cpuText() {
            ...
            int percent = (int) Math.round(processCpuLoad * 100.0); // meaning
            return percent + "%";
        }
    }
}
