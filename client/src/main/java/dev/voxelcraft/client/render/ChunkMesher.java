...
public final class ChunkMesher {
    ...
    private final SnapshotBlockArrayPool snapshotBlockPool = new SnapshotBlockArrayPool(4);
            ...
            for (int exY = 0; exY < expandedHeight; exY++) {
                ...
                for (int exZ = 0; exZ < SNAPSHOT_XZ; exZ++) {
                    ...
                    for (int exX = 0; exX < SNAPSHOT_XZ; exX++) {
                        ...
                        // 中文标注（局部变量）：`neighborIndex`，含义：用于表示邻居、索引。
                        int neighborIndex = neighborRowBase + chunkXIndex;
                        // 中文标注（局部变量）：`sourceChunk`，含义：用于表示source、区块。
                        Chunk sourceChunk = snapshotNeighbors[neighborIndex];
                        // 中文标注（局部变量）：`sourceSection`，含义：用于表示source、分段。
                        Section sourceSection = snapshotNeighborSections[neighborIndex];
