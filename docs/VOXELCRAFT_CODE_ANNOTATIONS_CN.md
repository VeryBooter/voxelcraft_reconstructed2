...
- `client/src/main/java/dev/voxelcraft/client/VoxelcraftClientApp.java`
  - 启动配置、软件/GPU 模式选择、headless 回退
- `client/src/main/java/dev/voxelcraft/client/runtime/GpuClientRuntime.java`
  - GLFW 窗口循环 + 输入桥接
- `client/src/main/java/dev/voxelcraft/client/render/GpuChunkRenderer.java`
  - OpenGL GPU 渲染实现
...
2. GPU 渲染目前使用 immediate mode，后续建议升级为 VBO/VAO + shader pipeline。
